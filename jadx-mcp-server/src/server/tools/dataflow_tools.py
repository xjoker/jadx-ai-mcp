...
#
# Each category entry is a dict with:
#   patterns  : list[str]  — substrings to match against method labels
#   severity  : str        — "high" | "medium" | "low"
#
# For backward compatibility the top-level dict is also exposed as a flat
# string-list dict via _SINK_PATTERNS (used by _match_sink).
# ---------------------------------------------------------------------------
...
_SINK_DEFINITIONS: dict[str, dict] = {
    "network": {
        ...
        "patterns": [
            ...
            # Volley
            "RequestQueue.add",
            # gRPC (stub. prefix — any stub method call)
            "stub.",
        ],
    },
    "crypto": {
        ...
        "patterns": [
            ...
            # KeyStore
            "KeyStore.setEntry", "KeyStore.setKeyEntry",
            # BiometricPrompt
    ...
    "storage": {
        ...
        "patterns": [
            ...
            # Room annotations (pattern-matched in method names generated by Room)
    ...
    "ipc": {
        ...
        "patterns": [
            ...
            # ContentResolver write operations
    ...
    "background": {
        ...
        "patterns": [
            # WorkManager
            "WorkManager.enqueue", "WorkManager.enqueueUniqueWork",
            # JobScheduler
...
# Flat dict for _match_sink backward compatibility
_SINK_PATTERNS: dict[str, list[str]] = {
    category: info["patterns"] for category, info in _SINK_DEFINITIONS.items()
}
...
def _match_sink(method_label: str) -> tuple[bool, Optional[str], Optional[str]]:
    """Check whether a method label matches a known sink pattern.

    Returns:
        (matched, sink_type, severity)
    """
...
async def trace_data_flow(
    ...
    while queue:
        ...
        for callee in callees:
            ...
            is_sink, sink_type, sink_severity = _match_sink(label)
            child_node: dict = {
                "method": label,
                "full_class": c_class,
                "depth": depth,
